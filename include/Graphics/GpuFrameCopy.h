 ...
 */
/**
 * @param rowPitchBytes  Number of bytes between two consecutive rows in the GPU
 *                       buffer. This comes from Vulkan's VkSubresourceLayout.
 */
/**
 * Copy a GPU buffer containing packed 10-bit macropixels (Y0 U Y1 V) into a
 * planar AVFrame. Each macropixel is stored in two little-endian dwords and
 * represents two pixels of 4:2:2 video. The AVFrame must use
 * AV_PIX_FMT_YUV422P10LE format.
 *
 * @param mappedGpuMemory Pointer to the start of the GPU buffer.
 * @param videoWidth      Frame width in pixels (must be even).
 * @param videoHeight     Frame height in pixels.
 * @param frame           Destination AVFrame already allocated for
 *                        AV_PIX_FMT_YUV422P10LE.
 * @param validate        If true and PRORES_GPU_VALIDATE is defined, validate
 *                        the very first macropixel and return false on
 *                        mismatch.
 */
