...
AVCodecContext* create_dnxhr_hqx_encoder(int width, int height, AVRational time_base) {
    ...
    LogDnxhr(std::string("[DNxHR] DNxHR HQX init: ") + std::to_string(width) +
             "x" + std::to_string(height) + ", pix_fmt=" +
             av_get_pix_fmt_name(c->pix_fmt) + ", profile=" +
             std::to_string(c->profile));
