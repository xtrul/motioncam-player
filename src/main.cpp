...
int main(int argc, char* argv[]) {
    ...
    // Parse command-line arguments for --new-instance
    ...
    for (int i = 1; i < argc; ++i) {
        if (argv[i] != nullptr) {
            ...
            if (arg == "--new-instance") {
                ...
                LogToFile("[main] --new-instance flag detected. Will skip single instance check.");
            } else if (inPath.empty() && arg.find("--") != 0) {
    ...
    if (!forceNewInstance) {
        ...
        if (instanceGuard.alreadyRunning()) {
            ...
            if (!inPath.empty()) {
                ...
                if (hwnd) {
                    LogToFile(std::string("[main] Found existing instance window (HWND: ") + std::to_string(reinterpret_cast<uintptr_t>(hwnd)) + "). Sending file: " + inPath);
                ...
                LogToFile("[main] No file argument to forward.");
            }
        ...
        LogToFile("[main] Skipping single instance check due to --new-instance flag.");
    }
#endif
