                         ...
                         double black, double white)
{
    ...
    auto to8=[&](uint16_t v){
        double norm = (static_cast<double>(v) - black) / (white - black);
...
bool generateThumbnail(const std::string& mcrawPath, const std::string& outPng)
{
    try {
        ...
        const nlohmann::json& contMeta = dec.getContainerMetadata();
        double black = 0.0;
        double white = 65535.0;
        try {
            if(contMeta.contains("blackLevel")) {
                if(contMeta["blackLevel"].is_number())
                    black = contMeta["blackLevel"].get<double>();
                else if(contMeta["blackLevel"].is_array() && !contMeta["blackLevel"].empty())
                    black = contMeta["blackLevel"][0].get<double>();
            }
            if(contMeta.contains("whiteLevel")) {
                if(contMeta["whiteLevel"].is_number())
                    white = contMeta["whiteLevel"].get<double>();
            }
        } catch(const std::exception&) {}

        std::vector<uint8_t> rgb(width*height*3);
        for(int y=0;y<height;++y) for(int x=0;x<width;++x){
            RGB px = bilinearPixel(raw,width,height,x,y,cfa,black,white);
