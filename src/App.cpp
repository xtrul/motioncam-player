        ...
        if (!dec_frames.empty()) {
            ...
            try {
                ...
                if (m_rendererVk) {
                    ...
                    if (meta_orient) {
                        int val = *meta_orient;
                        if (val == 6 || val == 90) orient = 1;         // 90° CW
                        else if (val == 3 || val == 180) orient = 2;    // 180°
                        else if (val == 8 || val == 270) orient = 3;    // 270° CW
                        else if (val == 0 || val == 1) orient = 0;      // Normal
                        else orient = ((val % 360) / 90) % 4;           // Fallback
                    } else if (local_raw_width < local_raw_height) {
                        orient = 1; // Assume portrait if no metadata
                    }
