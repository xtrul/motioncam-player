// --- START OF FILE src/Renderer_VK.cpp ---
...
Renderer_VK::Renderer_VK(VkPhysicalDevice physicalDevice, VkDevice device, VmaAllocator allocator, VkQueue graphicsQueue, VkCommandPool commandPool)
    ...
    m_stagingBufferMappedData(nullptr),
...
Renderer_VK::~Renderer_VK() {
    LogToFile("[Renderer_VK] Destructor called. Cleanup should be called explicitly before this.");
    std::cout << "[Renderer_VK] Destructor called. Cleanup should be called explicitly before this." << std::endl;
}
...
void Renderer_VK::cleanup() {
    ...
    if (m_stagingBuffer != VK_NULL_HANDLE) {
        LogToFile("[Renderer_VK::cleanup] Destroying staging buffer.");
        std::cout << "[Renderer_VK::cleanup] Destroying staging buffer." << std::endl;
...
void Renderer_VK::onSwapChainRecreated(VkRenderPass renderPass, uint32_t swapChainImageCount) {
    ...
    if (!createGraphicsPipeline(renderPass)) { LogToFile("[Renderer_VK::onSwapChainRecreated] ERROR: Failed to recreate graphics pipeline"); throw std::runtime_error("Failed to recreate graphics pipeline"); }
    if (!createUniformBuffers()) { LogToFile("[Renderer_VK::onSwapChainRecreated] ERROR: Failed to recreate uniform buffers"); throw std::runtime_error("Failed to recreate uniform buffers"); }
    if (!createDescriptorPool()) { LogToFile("[Renderer_VK::onSwapChainRecreated] ERROR: Failed to recreate descriptor pool"); throw std::runtime_error("Failed to recreate descriptor pool"); }
    if (!createDescriptorSets()) { LogToFile("[Renderer_VK::onSwapChainRecreated] ERROR: Failed to recreate descriptor sets"); throw std::runtime_error("Failed to recreate descriptor sets"); }
...
// CORRECTED: Implementation for ensureRawImageReadyAndUpload
void Renderer_VK::ensureRawImageReadyAndUpload(VkCommandBuffer commandBuffer,
                                               const std::vector<uint16_t>& rawData,
                                               int width, int height,
                                               uint32_t currentFrameIndex) {
    if (width <= 0 || height <= 0 || rawData.empty()) {
        LogToFile("[Renderer_VK::ensureRawImageReadyAndUpload] ERROR: Invalid data or dimensions.");
        return;
    }

    bool needsResize = (width != m_currentRawW || height != m_currentRawH);

    if (needsResize) {
        LogToFile(std::string("[Renderer_VK::ensureRawImageReadyAndUpload] Raw image dimensions changed. Recreating. Old:")
            + std::to_string(m_currentRawW) + "x" + std::to_string(m_currentRawH) + " New:" + std::to_string(width) + "x" + std::to_string(height));
        
        VK_CHECK(vkDeviceWaitIdle(m_device));

        if (!createRawImageResources(width, height)) {
            LogToFile("[Renderer_VK::ensureRawImageReadyAndUpload] ERROR: Failed to recreate raw image resources for new dimensions.");
            throw std::runtime_error("Failed to recreate raw image resources for new dimensions.");
        }

        if (m_descriptorPool != VK_NULL_HANDLE && !m_descriptorSets.empty() && m_descriptorSets.size() == m_swapChainImageCount) {
            LogToFile("[Renderer_VK::ensureRawImageReadyAndUpload] Updating all descriptor sets for new raw image view.");
            for (size_t i = 0; i < m_swapChainImageCount; ++i) {
             ...
             LogToFile("[Renderer_VK::ensureRawImageReadyAndUpload] WARNING: Descriptors not updated after resize due to pool/sets state.");
        }
        m_currentRawW = width;
        m_currentRawH = height;
    }
    recordUploadRawTextureData(commandBuffer, rawData, width, height);
}

// CORRECTED: Implementation for recordUploadRawTextureData
void Renderer_VK::recordUploadRawTextureData(VkCommandBuffer commandBuffer, const std::vector<uint16_t>& rawData, int width, int height) {
    if (width <= 0 || height <= 0 || rawData.empty()) {
        LogToFile("[Renderer_VK::recordUploadRawTextureData] ERROR: Invalid data or dimensions for upload.");
        return;
    }

    VkDeviceSize requiredStagingBufferSize = static_cast<VkDeviceSize>(width) * height * sizeof(uint16_t);

    if (m_stagingBuffer == VK_NULL_HANDLE || m_stagingBufferSize < requiredStagingBufferSize) {
        if (m_stagingBuffer != VK_NULL_HANDLE) {
            vmaDestroyBuffer(m_allocator, m_stagingBuffer, m_stagingBufferAllocation);
        }
        ...
        bufferInfo.size = requiredStagingBufferSize;
        ...
        VK_CHECK(vmaCreateBuffer(m_allocator, &bufferInfo, &allocCreateInfo, &m_stagingBuffer, &m_stagingBufferAllocation, &stagingAllocInfoResult));
        m_stagingBufferMappedData = stagingAllocInfoResult.pMappedData;
        m_stagingBufferSize = requiredStagingBufferSize;
        LogToFile(std::string("[Renderer_VK::recordUploadRawTextureData] Staging buffer (re)created. Size: ") + std::to_string(m_stagingBufferSize));
    }

    memcpy(m_stagingBufferMappedData, rawData.data(), requiredStagingBufferSize);
    
    // If memory is not HOST_COHERENT, a flush is needed
    // VmaAllocationInfo allocInfoCheck;
    // vmaGetAllocationInfo(m_allocator, m_stagingBufferAllocation, &allocInfoCheck);
    // VkMemoryPropertyFlags memFlags;
    // vmaGetMemoryTypeProperties(m_allocator, allocInfoCheck.memoryType, &memFlags);
    // if (!(memFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) {
    //     VK_CHECK(vmaFlushAllocation(m_allocator, m_stagingBufferAllocation, 0, VK_WHOLE_SIZE));
    // }
    ...
    vkCmdCopyBufferToImage(commandBuffer, m_stagingBuffer, m_rawImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
    ...
    vkCmdPipelineBarrier(commandBuffer,
                         ...
                         0, 0, nullptr, 0, nullptr, 1, &barrierToShaderRead);
}
...
bool Renderer_VK::createUniformBuffers() {
    ...
    for (size_t i = 0; i < m_swapChainImageCount; i++) {
        ...
        VK_CHECK(vmaCreateBuffer(m_allocator, &bufferInfo, &allocInfo, &m_uniformBuffers[i], &m_uniformBufferAllocations[i], &allocationInfoResult));
...
bool Renderer_VK::createGraphicsPipeline(VkRenderPass renderPass) {
    ...
    LogToFile(std::string("[Renderer_VK::createGraphicsPipeline] Using vertShaderPath: ") + vertShaderPath);
    LogToFile(std::string("[Renderer_VK::createGraphicsPipeline] Using fragShaderPath: ") + fragShaderPath);
...
void Renderer_VK::updateUniformBuffer(uint32_t currentImageIndex, const ShaderParamsUBO& ubo) {
    ...
    // Ensure data is visible to the GPU by flushing the allocation
    // if the memory type is not HOST_COHERENT.
    VmaAllocationInfo allocInfo;
    vmaGetAllocationInfo(m_allocator, m_uniformBufferAllocations[currentImageIndex], &allocInfo);
    
    VkMemoryPropertyFlags memFlags;
    vmaGetMemoryTypeProperties(m_allocator, allocInfo.memoryType, &memFlags);

    if (!(memFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) {
        // LogToFile(std::string("[Renderer_VK::updateUniformBuffer] Flushing UBO for frame ") + std::to_string(currentImageIndex)); // Optional: for debugging
        VK_CHECK(vmaFlushAllocation(m_allocator, m_uniformBufferAllocations[currentImageIndex], 0, VK_WHOLE_SIZE));
    }
}
...
// CORRECTED: Signature of recordRenderCommands
void Renderer_VK::recordRenderCommands(VkCommandBuffer commandBuffer, uint32_t currentFrameIndex,
    ...
    if (w <= 0 || h <= 0 ) { // rawData check removed as it's no longer passed
        LogToFile("[Renderer_VK::recordRenderCommands] Skipping draw, raw image not ready or dimensions invalid.");
        return;
    }
    ...
    if (currentFrameIndex < m_descriptorSets.size()) {
        ...
        vkCmdDraw(commandBuffer, 6, 1, 0, 0); // Draw fullscreen quad (6 vertices)
    }
    else {
...
VkCommandBuffer Renderer_VK::beginSingleTimeCommands() {
    ...
    allocInfo.commandPool = m_hostSiteCommandPool; // Use the App's command pool
...
void Renderer_VK::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
    ...
    VK_CHECK(vkQueueWaitIdle(m_graphicsQueue)); // Wait for the commands to complete
...
void Renderer_VK::resetDimensions() {
    m_currentRawW = 0; // This will trigger a resize on next frame if needed
    m_currentRawH = 0;
}
