        ...
        ImGui::BeginChild("Footer", ImVec2(0, 0), false, ImGuiWindowFlags_NoScrollbar);
        {
            ...
            ImGui::BeginChild("FilesPanel_Footer", ImVec2(fileListWidth, 0), true);
            {
                ...
                if (ImGui::Button("Remove") && appInstance->m_selectedBatchIndex != -1) {
                    ...
                    if (appInstance->m_selectedBatchIndex >= (int)appInstance->m_fileList.size()) {
                        appInstance->m_selectedBatchIndex = static_cast<int>(appInstance->m_fileList.size()) - 1;
                    }
                    if (appInstance->m_selectedBatchIndex >= 0) {
                        appInstance->loadFileAtIndex(appInstance->m_selectedBatchIndex);
                    } else {
                        appInstance->m_currentFileIndex = -1;
                    }
                }
                ImGui::SameLine();
            ...
            ImGui::BeginChild("RenderOptionsPanel", ImVec2(0, 0), true);
            {
                ...
                if (ImGui::Button("Select")) {
                    ...
                    if (!folder.empty()) {
                        ...
                        appInstance->m_outputFolder[sizeof(appInstance->m_outputFolder) - 1] = '\0';
                    }
                }
                ImGui::Separator();
                ImGui::Text("3) Click start to render all RAW containers");
                if (ImGui::Button("Render", ImVec2(-1, 40))) {
                    if (!appInstance->m_batchActive.load()) {
