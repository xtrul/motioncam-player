    ...
    void render(App* appInstance) {
        ...
        if (ui.showMetrics) {
            ...
            if (ImGui::Begin("METRICS", &appInstance->m_showMetrics, metrics_window_flags)) {
                ...
                // Black/White Level Presets
                const char* levelPresetItems[] = {
                    "Dynamic", "Static", "1023/64", "4095/256", "16383/1024",
                    "65535/4096", "4095/64", "16383/64", "16383/0"
                };
                int levelPresetTemp = static_cast<int>(appInstance->m_levelPreset);
                if (ImGui::Combo("Levels", &levelPresetTemp, levelPresetItems, IM_ARRAYSIZE(levelPresetItems))) {
                    appInstance->m_levelPreset = static_cast<App::LevelPreset>(levelPresetTemp);
                }
                ImGui::Text("  Black: %.0f, White: %.0f", appInstance->m_staticBlack, appInstance->m_staticWhite);

                // Gamma slider
                ImGui::SliderFloat("Gamma", &appInstance->m_gamma, 0.1f, 3.0f, "%.2f");
                if (ImGui::IsItemHovered()) {
                    ImGui::SetTooltip("Adjust midtones without clipping extremes\n(< 1.0 = brighter, > 1.0 = darker)");
                }
