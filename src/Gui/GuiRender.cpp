...
namespace GuiOverlay {
    ...
    void render(App* appInstance) {
        ...
        if (ImGui::Begin("Controls & Export")) {
            ...
            if (ImGui::SliderInt("##Seek", &current_frame_int, 0, static_cast<int>(total) > 0 ? static_cast<int>(total) - 1 : 0, "Frame %d")) {
...
#else
    void render(App* appInstance) {
        // This part is for the player, which you are moving away from. No changes needed.
        if (!appInstance) return;
        // ... (existing non-converter UI code)
    }
#endif
