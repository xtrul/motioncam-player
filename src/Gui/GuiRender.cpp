...
namespace GuiOverlay {
    ...
    void render(App* appInstance) {
        ...
        // If no file is loaded, show a prompt in the center of the main viewport.
        if (!appInstance->m_firstFileLoaded) {
            ...
            promptFlags |= ImGuiWindowFlags_NoDocking;
#endif
            ImGui::Begin("OPEN_PROMPT", nullptr, promptFlags);
            if (ImGui::Button("Add Files or Drag & Drop to Begin", ImVec2(300, 50))) {
                appInstance->triggerOpenFileViaDialog();
            }
            ImGui::End();
            // Reset preview rect when no file is loaded
        ...
        if (ImGui::Begin("Preview & Controls")) {
            // This child region is our Vulkan rendering target.
            ...
            // Playback Controls
            bool paused = true;
            if (appInstance->m_playbackController_ptr) paused = appInstance->m_playbackController_ptr->isPaused();
            ...
            if (ImGui::SliderInt("##Seek", &current_frame_int, 0, (total > 0) ? static_cast<int>(total) - 1 : 0, "Frame %d / %d")) {
                if(ImGui::IsItemActive()) appInstance->performSeek(static_cast<size_t>(current_frame_int));
            }
            ImGui::PopItemWidth();
