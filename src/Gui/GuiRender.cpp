...
namespace GuiOverlay {
    ...
    void render(App* appInstance) {
        ...
        if (ImGui::BeginPopup("AppContextMenu")) {
            ...
            if (ImGui::MenuItem("Export to DNxHR HQX (.mxf)", nullptr, false, canOperateOnCurrentFile)) {
                if (appInstance) appInstance->exportCurrentClipToDnxhr(false);
            }
            if (ImGui::MenuItem("Convert to DNxHR HQX (GPU)", nullptr, false, canOperateOnCurrentFile)) {
                if (appInstance) appInstance->convertCurrentClipToDnxhr();
            }
#else
            ImGui::MenuItem("Export to DNxHR (.mxf)", nullptr, false, false);
#endif
            ImGui::EndPopup();
        }
        ...
        if (ImGui::BeginPopupModal("EXPORT_PROGRESS", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
            int cur = appInstance->m_exportStatus.currentFrame.load();
            int total = appInstance->m_exportStatus.totalFrames;
            float progress = total > 0 ? static_cast<float>(cur) / static_cast<float>(total) : 0.0f;
            ImGui::Text("Exporting to DNxHR HQX (10-bit 4:2:2) %d / %d", cur, total);
            ImGui::ProgressBar(progress, ImVec2(200, 0));
            if (!appInstance->m_exportStatus.errorMsg.empty()) {
                ImGui::TextColored(ImVec4(1,0,0,1), "%s", appInstance->m_exportStatus.errorMsg.c_str());
            }
            if (!appInstance->m_exportStatus.active.load()) {
