...
namespace GuiOverlay {
    ...
    void render(App* appInstance) {
        ...
        ImGui::Begin("Preview", nullptr, fixed_flags);
        {
            ...
            const float barHeight = btnSize * 0.18f; // slightly thicker
            // --- POLISH END ---
            ...
            float iconSize = btnSize * 0.9f;
            ImVec2 iconPos(btnPos.x + (btnSize - iconSize) * 0.5f,
                           btnPos.y + (btnSize - iconSize) * 0.5f);
            dl->AddText(GuiStyles::G_AuxOverlayIconFont, iconSize, iconPos, ImGui::GetColorU32(ImGuiCol_Text), icon);
            // --- POLISH END ---
