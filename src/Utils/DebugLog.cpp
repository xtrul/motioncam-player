...
#   ifdef _WIN32
    ...
    for (const char* dll : dlls) {
        ...
        if (std::filesystem::exists(p)) {
            LogToFile(std::string("[FFmpeg] Found ") + dll);
        } else {
    ...
    if (allFound)
        LogToFile("[FFmpeg] All runtime DLLs present; ProRes export enabled.");
    else
        LogToFile("[FFmpeg] Some FFmpeg DLLs missing; export may fail.");
#   else
    LogToFile(std::string("[FFmpeg] avcodec version ") + std::to_string(avcodec_version()));
#   endif
#else
