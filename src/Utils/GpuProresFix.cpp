                        ...
                        AVFrame *f, bool validate)
{
    ...
    for (int y = 0; y < h; ++y) {
        auto *Y = reinterpret_cast<uint16_t*>(f->data[0] + y * f->linesize[0]);
        auto *U = reinterpret_cast<uint16_t*>(f->data[1] + y * f->linesize[1]);
        auto *V = reinterpret_cast<uint16_t*>(f->data[2] + y * f->linesize[2]);
        ...
        for (int x = 0; x < w; x += 2) {
            ...
            if (validate && !y && !x && !validateMacroPixel(&gpu[src])) {
