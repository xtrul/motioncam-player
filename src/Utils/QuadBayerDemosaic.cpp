                       ...
                       unsigned threads)
{
    ...
    auto interpG = [&](int x, int y)->float{
        return 0.25f*(lin(x+step,y)+lin(x-step,y)+lin(x,y+step)+lin(x,y-step));
    };
    auto interpH = [&](int x, int y)->float{
        return 0.5f*(lin(x+step,y)+lin(x-step,y));
    };
    auto interpV = [&](int x, int y)->float{
        return 0.5f*(lin(x,y+step)+lin(x,y-step));
    };
    auto interpD = [&](int x, int y)->float{
        return 0.25f*(lin(x+step,y+step)+lin(x-step,y+step)+lin(x+step,y-step)+lin(x-step,y-step));
    };
