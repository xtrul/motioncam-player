                        ...
                        bool validate)
{
    ...
    const uint8_t* gpuBase = static_cast<const uint8_t*>(mappedGpuMemory);
    const size_t rowBytes = static_cast<size_t>(layout.rowPitch);
    const int w = videoWidth;
    const int h = videoHeight;

    for (int y = 0; y < h; ++y) {
        const uint32_t* src = reinterpret_cast<const uint32_t*>(gpuBase + y * rowBytes);
        ...
        for (int x = 0; x < w; x += 2) {
            ...
            if (validate && y == 0 && x == 0) {
                char buf[128];
                snprintf(buf, sizeof(buf), "[GPU-CHECK] MP0 after unpack: Y0=%u U=%u Y1=%u V=%u", yRow[0] >> 6, uRow[0] >> 6, yRow[1] >> 6, vRow[0] >> 6);
                LogProRes(buf);
            }
        }
    }
