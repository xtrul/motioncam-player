...
int GpuYuvConverter::convert(const nlohmann::json& meta,double black,double white,int cfa){
    if(m_images.empty()) return -1;
    int idx = m_index;
...
VkImage GpuYuvConverter::getImage(int index) const {
    if(index < 0 || index >= m_ringSize) return VK_NULL_HANDLE;
