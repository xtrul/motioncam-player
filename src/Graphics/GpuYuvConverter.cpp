...
bool GpuYuvConverter::init(int width, int height) {
  ...
  fs::path shaderPath =
      fs::path(g_AppBasePath) / "shaders_spv" / "raw_to_yuv422.comp.spv";
  auto code = VulkanHelpers::readFile(shaderPath.string());
  VkShaderModule module =
      VulkanHelpers::createShaderModule(m_renderer->m_device_p, code);
  ...
  vkGetPhysicalDeviceQueueFamilyProperties(m_renderer->m_physicalDevice_p,
  ...
  VK_CHECK_RENDERER(
      vkCreateCommandPool(m_renderer->m_device_p, &ci, nullptr, &m_cmdPool));
  ...
  VK_CHECK_RENDERER(vkCreateDescriptorSetLayout(
      m_renderer->m_device_p, &layoutCI, nullptr, &m_setLayout));
  ...
  VK_CHECK_RENDERER(vkCreatePipelineLayout(m_renderer->m_device_p, &pli,
  ...
  VK_CHECK_RENDERER(vkCreateComputePipelines(
      m_renderer->m_device_p, VK_NULL_HANDLE, 1, &cp, nullptr, &m_pipeline));
  ...
  VK_CHECK_RENDERER(vkCreateDescriptorPool(m_renderer->m_device_p, &poolCI,
                                           nullptr, &m_descPool));
  ...
  VK_CHECK_RENDERER(
      vkAllocateDescriptorSets(m_renderer->m_device_p, &ai, &m_descSet));
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    imageInfo.extent.width =
        static_cast<uint32_t>(i == 0 ? width : (width + 1) / 2);
    ...
    VK_CHECK_RENDERER(vmaCreateImage(m_renderer->m_allocator_p, &imageInfo,
                                     &allocInfo, &m_planeImages[i],
                                     &m_planeAllocs[i], nullptr));
    ...
    VK_CHECK_RENDERER(vkCreateImageView(m_renderer->m_device_p, &viewInfo,
                                        nullptr, &m_planeViews[i]));

    ImageResource::transitionImageLayout(
        m_renderer->m_device_p, m_cmdPool, m_renderer->m_graphicsQueue_p,
        m_planeImages[i], VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);
  }
  ...
  VK_CHECK_RENDERER(vmaCreateImage(m_renderer->m_allocator_p, &rawInfo,
                                   &rawAllocInfo, &m_rawImage, &m_rawAlloc,
                                   nullptr));
  ...
  VK_CHECK_RENDERER(vkCreateImageView(m_renderer->m_device_p, &rawViewCI,
                                      nullptr, &m_rawView));
  ...
  VK_CHECK_RENDERER(vkCreateSampler(m_renderer->m_device_p, &samplerInfo,
                                    nullptr, &m_rawSampler));

  ImageResource::transitionImageLayout(
      m_renderer->m_device_p, m_cmdPool, m_renderer->m_graphicsQueue_p,
      m_rawImage, VK_IMAGE_LAYOUT_UNDEFINED,
...
void GpuYuvConverter::cleanup() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (m_planeImages[i] != VK_NULL_HANDLE) {
      vmaDestroyImage(m_renderer->m_allocator_p, m_planeImages[i],
                      m_planeAllocs[i]);
...
bool GpuYuvConverter::convertToFrame(const uint16_t *raw, int width, int height,
  ...
  VkDeviceSize rawSize =
      static_cast<VkDeviceSize>(width) * height * sizeof(uint16_t);
  VkDeviceSize ySize =
      static_cast<VkDeviceSize>(width) * height * sizeof(uint16_t);
  VkDeviceSize cSize =
      static_cast<VkDeviceSize>((width / 2)) * height * sizeof(uint16_t);
  ...
  VK_CHECK_RENDERER(vmaCreateBuffer(m_renderer->m_allocator_p, &sbInfo,
                                    &sbAlloc, &stagingBuf, &stagingAlloc,
                                    &sbAllocInfo));
  ...
  VmaAllocation readbackAlloc[3]{VK_NULL_HANDLE, VK_NULL_HANDLE,
                                 VK_NULL_HANDLE};
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    rbAlloc.flags = VMA_ALLOCATION_CREATE_HOST_ACCESS_RANDOM_BIT |
                    VMA_ALLOCATION_CREATE_MAPPED_BIT;
    VK_CHECK_RENDERER(vmaCreateBuffer(m_renderer->m_allocator_p, &rbInfo,
                                      &rbAlloc, &readbackBuf[i],
                                      &readbackAlloc[i], &rbAllocInfo[i]));
  }

  VkCommandBuffer cmd =
      VulkanHelpers::beginSingleTimeCommands(m_renderer->m_device_p, m_cmdPool);
  ...
  vkCmdCopyBufferToImage(cmd, stagingBuf, m_rawImage,
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    rbRegion.imageExtent = {
        static_cast<uint32_t>(i == 0 ? width : (width + 1) / 2),
        static_cast<uint32_t>(height), 1};
    vkCmdCopyImageToBuffer(cmd, m_planeImages[i],
                           VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, readbackBuf[i],
                           1, &rbRegion);
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    vmaInvalidateAllocation(m_renderer->m_allocator_p, readbackAlloc[i], 0,
                            size);
  }
  ...
  if (frame->linesize[0] == (int)srcPitchY) {
    ...
    for (int y = 0; y < height; ++y) {
      memcpy(frame->data[0] + y * frame->linesize[0],
             reinterpret_cast<const uint8_t *>(rbAllocInfo[0].pMappedData) +
                 y * srcPitchY,
             srcPitchY);
    }
  }
  if (frame->linesize[1] == (int)srcPitchC) {
    ...
    for (int y = 0; y < height; ++y) {
      memcpy(frame->data[1] + y * frame->linesize[1],
             reinterpret_cast<const uint8_t *>(rbAllocInfo[1].pMappedData) +
                 y * srcPitchC,
             srcPitchC);
    }
  }
  if (frame->linesize[2] == (int)srcPitchC) {
    ...
    for (int y = 0; y < height; ++y) {
      memcpy(frame->data[2] + y * frame->linesize[2],
             reinterpret_cast<const uint8_t *>(rbAllocInfo[2].pMappedData) +
                 y * srcPitchC,
             srcPitchC);
    }
  }
  ...
  if (!dumped) {
    for (int i = 0; i < 16 && i < width / 2; i++) {
      const uint8_t *yRow = frame->data[0];
      const uint8_t *uRow = frame->data[1];
      const uint8_t *vRow = frame->data[2];
      uint16_t y0 = *reinterpret_cast<const uint16_t *>(yRow + i * 4);
      uint16_t y1 = *reinterpret_cast<const uint16_t *>(yRow + i * 4 + 2);
      uint16_t u = *reinterpret_cast<const uint16_t *>(uRow + i * 2);
      uint16_t v = *reinterpret_cast<const uint16_t *>(vRow + i * 2);
      std::ostringstream oss;
      oss << "[GPU-CHECK] MP" << i << " Y0=" << y0 << " U=" << u << " Y1=" << y1
          << " V=" << v;
      LogProRes(oss.str());
    }
    dumped = true;
  ...
  for (int i = 0; i < 3; ++i) {
    vmaDestroyBuffer(m_renderer->m_allocator_p, readbackBuf[i],
                     readbackAlloc[i]);
  }
  return true;
}
