...
bool GpuYuvConverter::convertAndReadback(const uint16_t* raw, int width, int height,
    ...
    LogProRes(std::string("[GPU] parameters w=") + std::to_string(width) + " h=" + std::to_string(height));
    ...
    LogProRes(std::string("[GPU] readback complete rowPitch=") + std::to_string(rowPitch));
