...
bool GpuDctQuantizer::process(VkImage luma, VkImage chromaU, VkImage chromaV, uint32_t* outBuf, size_t bufSize){
    ...
    if(luma == VK_NULL_HANDLE || chromaU == VK_NULL_HANDLE || chromaV == VK_NULL_HANDLE){
        // CPU fallback when no GPU images are provided. Useful for testing the
        // slice pipeline without GPU YUV resources.
        if(bufSize < required){
            LogDnxhr("[GpuDctQuantizer] buffer too small, required=" + std::to_string(required));
            return false;
        }
        memset(outBuf, 0, required);
        LogDnxhr("[GpuDctQuantizer] CPU fallback used");
        return true;
    }
