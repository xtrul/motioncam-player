...
void Renderer_VK::updateUniformBuffer(uint32_t uboBindingIndex, const ShaderParamsUBO& ubo) {
    ...
    if (!loggedOnce) {
        LogToFile("[Renderer_VK] ShaderParamsUBO size: " + std::to_string(sizeof(ubo)) + " bytes, gamma offset: " + std::to_string(offsetof(ShaderParamsUBO, gamma)));
        loggedOnce = true;
    }
...
void Renderer_VK::prepareAndUploadFrameData(
    ...
    if (useDynamicLevels && frameMetadata.contains("dynamicBlackLevel")) {
    ...
    if (useDynamicLevels && frameMetadata.contains("dynamicWhiteLevel") && frameMetadata.at("dynamicWhiteLevel").is_number()) {
    ...
    if (frameCounter++ % 60 == 0) {
        LogToFile("[Renderer_VK] Gamma value: " + std::to_string(gamma));
    }
