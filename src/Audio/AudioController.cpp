...
void AudioController::updatePlayback(int64_t elapsedNsSinceSegmentStart) {
    ...
    while (chunksQueuedThisCall < MAX_CHUNKS_PER_UPDATE_CALL) {
        if (!m_hasCache) {
            ...
            // Skip audio chunks that are significantly before the current playback position
            // This is essential for proper audio sync after seeking
            // We use a threshold slightly before current time to avoid skipping too aggressively
            int64_t skipThresholdNs = elapsedNsSinceSegmentStart - (m_latencyNs * 2); // 2x latency buffer
            if (tempChunk.first < skipThresholdNs && originalAbsoluteTs != -1LL) {
                continue; // Skip this chunk and load the next one
            }
