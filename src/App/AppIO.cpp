...
void App::ioWorkerLoop() {
    ...
    while (!m_threadsShouldStop.load(std::memory_order_relaxed)) {
        ...
        try {
            ...
            // Upstream decoder returns decompressed 16-bit frame data + JSON metadata.
            threadLocalDecoder->loadFrame(ts, frameData, frameMetadata);

            packet.width = frameMetadata.value("width", 0);
            packet.height = frameMetadata.value("height", 0);
            packet.compressionType = 0; // Already decompressed by the decoder.
            packet.compressedPayload = std::move(frameData);

            const auto metadataString = frameMetadata.dump();
            packet.metadataPayload.assign(metadataString.begin(), metadataString.end());

            const size_t expectedBytes = static_cast<size_t>(packet.width) * packet.height * sizeof(uint16_t);
            payloadSuccess = packet.width > 0 && packet.height > 0 && packet.compressedPayload.size() == expectedBytes;

            if (!payloadSuccess) {
                LogToFile(std::string("[App::ioWorkerLoop] Frame payload size mismatch for TS ")
                    + std::to_string(ts) + ". Expected bytes: " + std::to_string(expectedBytes)
                    + ", got: " + std::to_string(packet.compressedPayload.size())
                    + ", width: " + std::to_string(packet.width)
                    + ", height: " + std::to_string(packet.height));
            }
        }
        catch (const std::exception& e) {
            LogToFile(std::string("[App::ioWorkerLoop] EXCEPTION in loadFrame for TS ") + std::to_string(ts) + " (idx " + std::to_string(frameIndexInCurrentFile_io) + "): " + e.what());
...
void App::performSeek(size_t new_frame_index) {
    ...
    if (m_audio && m_decoderWrapper_ptr && m_decoderWrapper_ptr->getDecoder()) {
        ...
        if (freshAudioLoader) {
            // For seek, the audio anchor should be the first frame timestamp of the segment, 
            // not the current frame, to maintain proper sync with elapsed time calculations
            std::optional<int64_t> firstFrameMediaTsOpt = m_playbackController_ptr->getFirstFrameMediaTimestampOfSegment();
            if (firstFrameMediaTsOpt.has_value()) {
                LogToFile(std::string("[App::performSeek] -> AudioController::reset with segment's first video frame TS: ") + std::to_string(firstFrameMediaTsOpt.value()));
                m_audio->reset(freshAudioLoader, firstFrameMediaTsOpt.value());
            }
            else {
                LogToFile("[App::performSeek] WARNING: firstFrameMediaTsOpt was null during seek for audio reset. Using 0.");
                m_audio->reset(freshAudioLoader, 0);
            }
            if (m_playbackController_ptr) {
                m_audio->setPaused(m_playbackController_ptr->isPaused());
...
void App::anchorPlaybackTimeForResume() {
    ...
    if (m_audio && m_decoderWrapper_ptr && m_decoderWrapper_ptr->getDecoder()) {
        auto* freshAudioLoader = m_decoderWrapper_ptr->makeFreshAudioLoader();
        if (freshAudioLoader) {
            // For resume, the audio anchor should be the first frame timestamp of the segment, 
            // not the current frame, to maintain proper sync with elapsed time calculations
            if (firstFrameMediaTsOpt.has_value()) {
                LogToFile(std::string("[App::anchorPlaybackTimeForResume] -> AudioController::reset with segment's first video frame TS: ") + std::to_string(firstFrameMediaTsOpt.value()));
                m_audio->reset(freshAudioLoader, firstFrameMediaTsOpt.value());
            }
            else {
                LogToFile("[App::anchorPlaybackTimeForResume] WARNING: firstFrameMediaTsOpt was null for audio reset on resume. Using 0.");
                m_audio->reset(freshAudioLoader, 0);
            }
            if (m_playbackController_ptr) {
                m_audio->setPaused(m_playbackController_ptr->isPaused());
...
void App::softDeleteCurrentFile() {
    ...
    // Give the IO thread time to release the file handle
    ...
    try {
        ...
        // Try to rename/move the file with retries in case of temporary lock
        bool moveSucceeded = false;
        int retryCount = 0;
        const int maxRetries = 5;
        
        while (!moveSucceeded && retryCount < maxRetries) {
            try {
                ...
                if (retryCount < maxRetries) {
                    LogToFile(std::string("[App::softDeleteCurrentFile] Retry ") + std::to_string(retryCount) + "/" + std::to_string(maxRetries) + 
                              " - Failed to move file (may be locked): " + e.what());
