...
void App::exportCurrentClipToDnxhr() {
    ...
    LogDnxhr(std::string("[DNxHRExport] Total frames: ") + std::to_string(m_dnxhrStatus.totalFrames));
    ...
    m_dnxhrThread = std::thread([this, outputPath, useGpu]() {
        ...
        LogDnxhr("[DNxHRExport] Thread started");
        LogDnxhr(std::string("[DNxHRExport] MODE = ") + (useGpu ? "GPU (Vulkan hw_frames)" : "CPU (swscale)"));
        ...
        LogDnxhr(std::string("[DNxHRExport] Thread type: FRAME"));
        ...
        for (size_t idx = 0; idx < frames.size(); ++idx) {
            ...
            t0 = std::chrono::steady_clock::now();
            if (avcodec_send_frame(vctx, frame) < 0) { m_dnxhrStatus.errorMsg = "send_frame failed"; break; }
            while (avcodec_receive_packet(vctx, &pkt) == 0) {
                pkt.stream_index = vstream->index;
                pkt.duration = 1;
                pkt.pts = av_rescale_q(pkt.pts, vctx->time_base, vstream->time_base);
                pkt.dts = pkt.pts;
                if (av_interleaved_write_frame(fmt, &pkt) < 0) { m_dnxhrStatus.errorMsg = "write_frame failed"; av_packet_unref(&pkt); break; }
                av_packet_unref(&pkt);
                ++framesEncoded;
            }
            t1 = std::chrono::steady_clock::now();
        ...
        if (m_dnxhrStatus.errorMsg.empty()) {
            ...
            LogDnxhr(std::string("[DNxHRExport] Error: ") + m_dnxhrStatus.errorMsg);
        }
        m_dnxhrStatus.active.store(false);
        g_useGpuDnxhr = false;
    });
