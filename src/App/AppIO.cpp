...
void App::exportCurrentClipToProRes() {
    ...
    LogProRes(std::string("Starting export to ") + outputPath);
    ...
    m_proResThread = std::thread([this, outputPath]() {
        ...
        try {
            ...
            LogProRes(std::string("Failed to load first frame: ") + e.what());
        ...
        LogProRes(std::string("Frame size ") + std::to_string(width) + "x" + std::to_string(height) + ", total frames " + std::to_string(frames.size()));
        ...
        if (!vcodec) {
            LogProRes("ProRes encoder not found");
        ...
        if (avcodec_open2(vctx, vcodec, nullptr) < 0) {
            LogProRes("avcodec_open2 for video failed");
        ...
        LogProRes("Video encoder opened");
        ...
        if (avformat_write_header(fmt, nullptr) < 0) {
            LogProRes("avformat_write_header failed");
        ...
        LogProRes("Output header written");
        ...
        for (size_t idx = 0; idx < frames.size(); ++idx) {
            ...
            if((idx % 30)==0) LogProRes(std::string("Encoded frame ") + std::to_string(idx+1));
        }
        ...
        if (actx && astream) {
            ...
            while (loader && loader->next(chunk)) {
                ...
                if ((audioPts % (48000*2)) == 0) LogProRes(std::string("Audio chunk at sample ") + std::to_string(audioPts));
        ...
        if (m_proResStatus.errorMsg.empty()) {
            ...
            LogProRes("Export finished successfully");
        } else {
            LogProRes(std::string("Error: ") + m_proResStatus.errorMsg);
