        ...
        for (size_t idx = 0; idx < frames.size(); ++idx) {
            ...
            if (gpuActive) {
                ...
                for (int y=0; y<height; ++y) {
                    uint16_t* rowY = reinterpret_cast<uint16_t*>(frame->data[0] + y * frame->linesize[0]);
                    uint16_t* rowU = reinterpret_cast<uint16_t*>(frame->data[1] + y * frame->linesize[1]);
                    uint16_t* rowV = reinterpret_cast<uint16_t*>(frame->data[2] + y * frame->linesize[2]);
                    for (int x=0; x<width; x+=2) {
                        ...
                        rowY[x]     = static_cast<uint16_t>(Y0 << 6);
                        rowY[x + 1] = static_cast<uint16_t>(Y1 << 6);
                        rowU[x / 2] = static_cast<uint16_t>(U  << 6);
                        rowV[x / 2] = static_cast<uint16_t>(V  << 6);
                    }
                }
                if (idx == 0) {
                    std::ostringstream dbg;
                    dbg << "[GPU] AVFrame first: " << yPlane[0] << "," << yPlane[1]
                        << "," << uPlane[0] << "," << vPlane[0];
                    LogProRes(dbg.str());
                }
            } else {
