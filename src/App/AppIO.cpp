...
void App::exportCurrentClipToHEVC_AMD() {
    ...
    m_hevcThread = std::thread([this, outputPath]() {
        ...
        if (!vcodec) {
            m_hevcStatus.errorMsg = "AMD hardware encoder not available";
            m_hevcStatus.active.store(false);
            avformat_free_context(fmt);
        ...
        av_dict_set(&encOpts, "rc", "cqp", 0);
        av_dict_set(&encOpts, "cqp_i", "20", 0);
        av_dict_set(&encOpts, "cqp_p", "23", 0);
        av_dict_set(&encOpts, "cqp_b", "25", 0);
        if (avcodec_open2(vctx, vcodec, &encOpts) < 0) {
        ...
        av_dict_free(&encOpts);
        LogHevc("[HEVCExport] Video codec opened");
        avcodec_parameters_from_context(vstream->codecpar, vctx);
        vstream->time_base = timeBase;
        ...
        if (acodec) {
            ...
            if (avcodec_open2(actx, acodec, nullptr) >= 0) {
                avcodec_parameters_from_context(astream->codecpar, actx);
                astream->time_base = actx->time_base;
                LogHevc("[HEVCExport] Audio encoder initialized");
            } else {
                avcodec_free_context(&actx);
                actx = nullptr;
        ...
        int framesEncoded = 0;
        for (size_t idx = 0; idx < frames.size(); ++idx) {
            if (!dec->loadFrame(frames[idx], rawBuf, meta)) break;
            const uint16_t* raw = reinterpret_cast<const uint16_t*>(rawBuf.data());
            convertRawToRGB24(raw, cpParams, rgbBuf, threads);
            const uint8_t* srcSlices[1] = { rgbBuf.data() };
            int srcStride[1] = { width*3 };
            sws_scale(sws, srcSlices, srcStride, 0, height, frame->data, frame->linesize);
            frame->pts = pts++;
