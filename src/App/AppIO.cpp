...
void App::exportCurrentClipToProRes() {
    ...
    m_proResThread = std::thread([this, outputPath]() {
        ...
        for (size_t idx = 0; idx < frames.size(); ++idx) {
            ...
            // naive grayscale export from Bayer
            const uint16_t* src = asU16(raw);
            for (int y = 0; y < height; ++y) {
                uint16_t* ydst = reinterpret_cast<uint16_t*>(frame->data[0] + y * frame->linesize[0]);
                uint16_t* udst = reinterpret_cast<uint16_t*>(frame->data[1] + (y/1) * frame->linesize[1]);
                uint16_t* vdst = reinterpret_cast<uint16_t*>(frame->data[2] + (y/1) * frame->linesize[2]);
                for (int x = 0; x < width; ++x) {
                    uint16_t val16 = src[y*width + x];
                    uint16_t val10 = val16 >> 6;
                    ydst[x] = val10 << 6;
                    if ((x & 1) == 0) {
                        udst[x/2] = 512 << 6;
                        vdst[x/2] = 512 << 6;
