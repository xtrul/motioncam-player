...
void App::exportCurrentClipToDnxhr() {
    ...
    m_exportThread = std::thread([this, outputPath, codec, useGpu]() {
        av_log_set_level(AV_LOG_ERROR);
        auto log = [&](const std::string& m){ if (codec==ExportCodec::DNXHR_HQ10) LogDnxhr(m); else LogProRes(m); };
        std::string prefix = (codec==ExportCodec::DNXHR_HQ10)?"[DNxHRExport] ":"[ProResExport] ";
        ...
        for (size_t idx = 0; idx < frames.size(); ++idx) {
            ...
            t0 = std::chrono::steady_clock::now();
            if (avcodec_send_frame(vctx, frame) < 0) { m_exportStatus.errorMsg = "send_frame failed"; break; }
...
void App::convertCurrentClipToDnxhr() {
    ...
    exportCurrentClip(ExportCodec::DNXHR_HQ10, true);
#else
    showActionMessage("FFmpeg support not built");
#endif
}
...
#ifdef ENABLE_DNXHR_EXPORT
    exportCurrentClip(ExportCodec::PRORES, false);
#else
    showActionMessage("FFmpeg support not built");
#endif
}
...
#ifdef ENABLE_DNXHR_EXPORT
    exportCurrentClip(ExportCodec::PRORES, true);
#else
    showActionMessage("FFmpeg support not built");
#endif
}
