...
void App::handleKey(int key, int mods) {
    ...
    // Reset UI activity on any key press
    resetUIActivity();

    if (key == GLFW_KEY_TAB && mods == 0) {
        m_showUI = !m_showUI;
        m_manualUIToggle = true; // User manually toggled UI
        resetUIActivity(); // Reset activity timer
...
void App::handleCursorPos(double xpos, double ypos) {
    // Check for mouse movement to show UI
    const double movement_threshold = 1.0; // pixels
    double dx = xpos - m_lastMouseX;
    double dy = ypos - m_lastMouseY;
    
    if (std::abs(dx) > movement_threshold || std::abs(dy) > movement_threshold) {
        resetUIActivity();
    }
...
void App::updateUIAutoHide() {
    // Don't auto-hide if user manually toggled UI off or on recently
    if (m_manualUIToggle) {
        return;
    }
    
    // Only auto-hide during playback
    if (!m_playbackController || m_playbackController->isPaused()) {
        return;
    }
    
    auto now = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - m_lastUIActivity).count();
    
    // Auto-hide UI after 10 seconds of inactivity during playback
    if (elapsed >= 10 && m_showUI) {
        m_showUI = false;
    }
}

void App::resetUIActivity() {
    m_lastUIActivity = std::chrono::steady_clock::now();
    
    // Show UI if it was auto-hidden (but not if user manually toggled it off)
    if (!m_showUI && !m_manualUIToggle) {
        m_showUI = true;
    }
    
    // Reset manual toggle flag when user interacts
    m_manualUIToggle = false;
}