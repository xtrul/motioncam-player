...
layout(binding = 1) uniform ShaderParams {
    ...
    float gamma; // Gamma correction (1.0 = linear, < 1.0 = brighter, > 1.0 = darker)
} params;
...
void main() {
    ...
    // --- GAMMA CORRECTION ---
    // Apply gamma correction to adjust midtones without clipping extremes
    // gamma = 1.0: no change (linear)
    // gamma < 1.0: brightens midtones
    // gamma > 1.0: darkens midtones
    vec3 col_gamma_corrected = pow(col_saturated, vec3(1.0 / params.gamma));
    col_gamma_corrected = clamp(col_gamma_corrected, 0.0, 1.0);
    // --- END GAMMA CORRECTION ---

    // sRGB EOTF and output (using the gamma corrected color)
    // This is correct if your swapchain is linear (_UNORM)
    outColor = vec4(srgb_eotf(col_gamma_corrected.r),
                    srgb_eotf(col_gamma_corrected.g),
                    srgb_eotf(col_gamma_corrected.b),
                    1.0);
}
