...
- The application searches for available HEVC encoders in FFmpeg.
  It tries `hevc_amf`, `hevc_nvenc`, and `libx265` in that order.
- Build FFmpeg with one of these encoders enabled (use vcpkg features
  such as `amf`, `nvcodec`, or `x265`).
- Exports are always 10-bit and configure high quality options for the
  selected encoder. Files can be queued via **Add to HEVC Batch**.
