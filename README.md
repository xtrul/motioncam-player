...
`motioncam_batcher` is a small GUI tool for batch converting `.mcraw` clips.
Drag video files onto the window to populate the file list. The interface
provides:

- **File List** – shows all loaded clips with Remove and Clear actions.
- **Preview** – displays a frame from the selected clip with a scrub slider.
- **Export Settings** – choose frame range and enable optional corrections.
- **Render Config** – select ProRes, DNxHR or HEVC, the transfer function,
  frame rate and whether GPU rendering is enabled.

After configuring options press **Start Rendering** to convert each file.
