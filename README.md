...
- Configure with `cmake -DENABLE_RAW_PLANE_DUMP=ON` or add `-DENABLE_RAW_PLANE_DUMP=1` (`/D ENABLE_RAW_PLANE_DUMP=1` on MSVC) to write each frame's Y, U and V planes to a file named `test_422p10.yuv` next to the exported movie.
