...
- (void)providePreviewForFileAtURL:(NSURL *)url completionHandler:(void (^)(QLPreviewReply *, NSError *))handler API_AVAILABLE(macos(12.0)) {
    @autoreleasepool {
        try {
            ...
            size_t frameIndex = frames.size() > 23 ? 23 : 0; // use 24th frame if possible
            decoder.loadFrame(frames[frameIndex], data, metadata);
            unsigned width = metadata["width"];
            unsigned height = metadata["height"];

            // Convert 16-bit grayscale to 8-bit sRGB for Quick Look
            std::vector<uint8_t> rgba(width * height * 4);
            for (size_t i = 0; i < width * height; ++i) {
                uint8_t v = static_cast<uint8_t>(data[i] >> 8);
                rgba[4 * i + 0] = v;
                rgba[4 * i + 1] = v;
                rgba[4 * i + 2] = v;
                rgba[4 * i + 3] = 0xFF;
            ...
            CGContextRef context = CGBitmapContextCreate(rgba.data(), width, height, bitsPerComponent, bytesPerRow, colorSpace, kCGImageAlphaPremultipliedLast | kCGBitmapByteOrder32Little);
